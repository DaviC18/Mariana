...
test("gera slots de 30 minutos quando não existem conflitos", () => {
	const timeMin = new Date("2026-09-21T09:00:00-03:00");
	const timeMax = new Date("2026-09-21T11:00:00-03:00");
	...
	assert.deepEqual(
		...
		[
			...
			},
			{
				...
				start: "2026-09-21T12:30:00.000Z",
			},
			{
...
test("remove slots que entram em conflito com períodos ocupados", () => {
	...
	const busyPeriods = [
		{
			end: "2026-09-21T10:30:00-03:00",
			start: "2026-09-21T09:30:00-03:00",
		},
	];
...
test("não considera conflito quando o evento começa exatamente no fim do slot", () => {
	const timeMin = new Date("2026-09-21T09:00:00-03:00");
	const timeMax = new Date("2026-09-21T10:00:00-03:00");

	const busyPeriods = [
		{
			end: "2026-09-21T10:00:00-03:00",
			start: "2026-09-21T09:30:00-03:00",
		},
	];
	...
	assert.equal(slots[0]?.start.toISOString(), "2026-09-21T12:00:00.000Z");

	assert.equal(slots[0]?.end.toISOString(), "2026-09-21T12:30:00.000Z");
});
